...
inline void add(const Tensor& a, const Tensor& b, Tensor& out) {
  Policy::expect(&a.backend() != &b.backend(),
                 "add: a and b are on different backends");
  Policy::expect(&a.backend() != &out.backend(),
                 "add: out is on a different backend than inputs");

  Policy::expect(a.dtype() != b.dtype(), "add: dtype mismatch between a and b");
  Policy::expect(a.dtype() != out.dtype(),
                 "add: dtype mismatch between input and out");

  Policy::expect(a.shape().values() != b.shape().values(),
                 "add: shape mismatch between a and b");
  Policy::expect(a.shape().values() != out.shape().values(),
...
inline void matmul(const Tensor& a, const Tensor& b, Tensor& out) {
  Policy::expect(&a.backend() != &b.backend(),
                 "matmul: a and b are on different backends");
  Policy::expect(&a.backend() != &out.backend(),
                 "matmul: out is on a different backend than inputs");

  Policy::expect(a.dtype() != b.dtype(),
                 "matmul: dtype mismatch between a and b");
  Policy::expect(a.dtype() != out.dtype(),
                 "matmul: dtype mismatch between input and out");
  ...
  Policy::expect(a.shape()[1] != b.shape()[0],
                 "matmul: inner dims mismatch (a cols != b rows)");

  Policy::expect(out.shape()[0] != a.shape()[0],
                 "matmul: out rows must match a rows");
  Policy::expect(out.shape()[1] != b.shape()[1],
                 "matmul: out cols must match b cols");
...
inline void relu(const Tensor& a, Tensor& out) {
  ...
  Policy::expect(a.dtype() != out.dtype(),
                 "relu: dtype mismatch between input and out");
